...
template<class Type>
matrix<Type> makeH_si_crcvm(Type sigma_obs, int n_dim) {
...
template<class Type>
matrix<Type> makeT_si_crcvm(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_si_crcvm(Type beta, Type sigma,Type omega, Type delta) {
...
template <class Type>
Type nllk_si_crcvm(objective_function<Type>* obj) {
    //======//
    // DATA //
    //======//
    ...
    vector<Type> tau=(1-coeff)*tau0+coeff*(delta0+(tau1-delta0)*(exp(kappa*cos(phi))-1)/(exp(kappa)-1));
    vector<Type> omega=-sin(phi)/tau*coeff;
    ...
    Z(0,0)=1;
    Z(1,1)=1;
    matrix<Type> H = makeH_si_crcvm(sigma_obs, n_dim);
    matrix<Type> T(4, 4);
    matrix<Type> Q(4, 4);
    matrix<Type> F(2, 2);
    F.setZero();
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_si_crcvm(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_si_crcvm(beta(i), sigma(i), omega(i), dtimes(i));

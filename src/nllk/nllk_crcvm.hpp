...
template<class Type>
matrix<Type> makeH_crcvm(Type sigma_obs, int n_dim) {
...
template<class Type>
matrix<Type> makeT_crcvm(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_crcvm(Type beta, Type sigma,Type omega, Type delta) {
...
Type nllk_crcvm(objective_function<Type>* obj) {
    ...
    DATA_VECTOR(Dshore); // computed distance to the boundary
    ...
    vector<Type> tau = 0.1+(tau0-0.1)/2*(1+tanh(lambda*(Dshore-D0)));
    ...
    Z(0,0)=1;
    Z(1,1)=1;
    matrix<Type> H = makeH_crcvm(sigma_obs, n_dim);
    matrix<Type> T(4, 4);
    matrix<Type> Q(4, 4);
    matrix<Type> F(2, 2);
    F.setZero();
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_crcvm(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_crcvm(beta(i), sigma(i), omega(i), dtimes(i));

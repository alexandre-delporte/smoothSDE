...
template<class Type>
matrix<Type> makeH_vm_crcvm(Type sigma_obs, int n_dim) {
...
template<class Type>
matrix<Type> makeT_vm_crcvm(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_vm_crcvm(Type beta, Type sigma,Type omega, Type delta) {
...
template <class Type>
Type nllk_vm_crcvm(objective_function<Type>* obj) {
    //======//
    // DATA //
    //======//
    ...
    Z(0,0)=1;
    Z(1,1)=1;
    matrix<Type> H = makeH_vm_crcvm(sigma_obs, n_dim);
    matrix<Type> T(4, 4);
    matrix<Type> Q(4, 4);
    matrix<Type> F(2, 2);
    F.setZero();
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_vm_crcvm(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_vm_crcvm(beta(i), sigma(i), omega(i), dtimes(i));

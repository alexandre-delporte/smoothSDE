...
template<class Type>
matrix<Type> makeH_racvm2(Type sigma_obs) {
    matrix<Type> H(2, 2);
    H.setZero();
    for(int i = 0; i < 2; i ++) {
...
template<class Type>
matrix<Type> makeT_racvm2(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_racvm2(Type beta, Type sigma,Type omega, Type delta) {
...
//' @param dt Length of time interval
template<class Type>
matrix<Type> makeB_racvm2(Type beta,Type omega, Type delta) {
    matrix<Type> B(4,2);
    B.setZero();
...
template <class Type>
Type nllk_racvm2(objective_function<Type>* obj) {
    //======//
    // DATA //
    //======//
    ...
    vector<Type> sigma = exp(par_mat.col(3).array());
    ...
    Z(0,0)=1;
    Z(1,1)=1;
    matrix<Type> H = makeH_racvm2(sigma_obs);
    matrix<Type> T(4, 4);
    matrix<Type> Q(4, 4);
    matrix<Type> F(2, 2);
    F.setZero();
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_racvm2(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_racvm2(beta(i), sigma(i), omega(i), dtimes(i));
            matrix<Type> B = makeB_racvm2(beta(i),omega(i),dtimes(i));

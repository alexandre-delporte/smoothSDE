...
template<class Type>
matrix<Type> makeH_s_crcvm2(Type sigma_obs, int n_dim) {
...
template<class Type>
matrix<Type> makeT_s_crcvm2(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_s_crcvm2(Type beta, Type sigma,Type omega, Type delta) {
...
template <class Type>
Type nllk_s_crcvm2(objective_function<Type>* obj) {
    //======//
    // DATA //
    //======//
    ...
    vector<Type> D0 = exp(par_mat.col(2).array());
    ...
    Z(0,0)=1;
    Z(1,1)=1;
    matrix<Type> H = makeH_s_crcvm2(sigma_obs, n_dim);
    matrix<Type> T(4, 4);
    matrix<Type> Q(4, 4);
    matrix<Type> F(2, 2);
    F.setZero();
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_s_crcvm2(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_s_crcvm2(beta(i), sigma(i), omega(i), dtimes(i));

...
template<class Type>
matrix<Type> makeH_racvm3(Type sigma_obs) {
    matrix<Type> H(2, 2);
    H.setZero();
    for(int i = 0; i < 2; i ++) {
...
template<class Type>
matrix<Type> makeT_racvm3(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_racvm3(Type beta, Type sigma,Type omega, Type delta) {
...
//' @param dt Length of time interval
template<class Type>
matrix<Type> makeB_racvm3(Type beta,Type omega, Type delta) {
    matrix<Type> B(4,2);
    B.setZero();
...
template <class Type>
Type nllk_racvm3(objective_function<Type>* obj) {
    //======//
    // DATA //
    //======//
    ...
    vector<Type> phi = par_mat.col(4).array();
    vector<Type> beta = 1/tau;
    vector<Type> sigma = 2 * nu / sqrt(M_PI * tau);
    vector<Type> omega = phi/tau;
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_racvm1(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_racvm1(beta(i), sigma(i), omega(i), dtimes(i));
            matrix<Type> B = makeB_racvm1(beta(i),omega(i),dtimes(i));

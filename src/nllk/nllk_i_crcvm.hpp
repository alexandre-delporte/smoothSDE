...
template<class Type>
matrix<Type> makeH_i_crcvm(Type sigma_obs, int n_dim) {
...
template<class Type>
matrix<Type> makeT_i_crcvm(Type beta, Type omega, Type delta) {
...
template<class Type>
matrix<Type> makeQ_i_crcvm(Type beta, Type sigma,Type omega, Type delta) {
...
template <class Type>
Type nllk_i_crcvm(objective_function<Type>* obj) {
    //======//
    // DATA //
    //======//
    ...
    vector<Type> tau1 = exp(par_mat.col(2).array());
    vector<Type> D0 = exp(par_mat.col(3).array());
    vector<Type> kappa = exp(par_mat.col(4).array());
    vector<Type> sigma = exp(par_mat.col(5).array());

    // Define vectors tau and omega based on conditions
    vector<Type> tau(par_mat.rows());
    vector<Type> omega(par_mat.rows());

    for (int i = 0; i < par_mat.rows(); ++i) {
        if (Dshore(i) > D0(i)) {
            tau(i) = tau0(i);
            omega(i)=0;
        } else {
            tau(i) = delta0(i) + (tau1(i) - delta0(i)) / (exp(kappa(i)) - Type(1)) * (exp(kappa(i) * cos(phi(i))) - Type(1));
            omega(i)=-sin(phi(i))/tau(i);
        }
    }
    ...
    Z(0,0)=1;
    Z(1,1)=1;
    matrix<Type> H = makeH_i_crcvm(sigma_obs, n_dim);
    matrix<Type> T(4, 4);
    matrix<Type> Q(4, 4);
    matrix<Type> F(2, 2);
    F.setZero();
    ...
    for(int i = 1; i < n; i++) {
        if(ID(i) != ID(i-1)) {
            ...
            matrix<Type> T = makeT_i_crcvm(beta(i),omega(i),dtimes(i));
            matrix<Type> Q = makeQ_i_crcvm(beta(i), sigma(i), omega(i), dtimes(i));
